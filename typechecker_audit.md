...
- **`TypeChecker::type_expr`** (~430 righe effettive nel corpo dello switch, su 1211 totali del file) viola il principio di singola responsabilità e il limite di complessità cognitiva del progetto (CCN ≤15, AGENTS.md §7). Gestisce generazione vincoli, type checking anticipato, e costruzione AST tipizzato simultaneamente.
...
`push_scope()` crea un nuovo `unordered_map` nel vector `scopes_`. `define()` inserisce nel back. `lookup()` itera in ordine inverso (dall'interno all'esterno) [`SymbolTable.cpp:20–26`]. `set_function_return_context()` cerca per nome poi fallback su qualsiasi function binding [`SymbolTable.cpp:37–53`].

#### 2.5 Critical Points

**`DEF-010` — `StringHash` con `string_view` e ownership**: `SymbolTable` usa `unordered_map<std::string_view, TypeScheme, StringHash>` [`SymbolTable.hpp:69`]. Le `string_view` come chiave puntano a stringhe esterne. Se la stringa originale viene deallocata, la chiave diventa dangling. Questo è sicuro finché i nomi degli identificatori vivono abbastanza (tipicamente da `std::string` nell'AST), ma è una **precondizione non documentata**.
...
**`DEF-012` — `set_function_return_context` cerca per nome poi per tipo**: La funzione cerca prima il binding per nome [`SymbolTable.cpp:39–45`], poi fallback su qualsiasi function binding nello scope corrente [`SymbolTable.cpp:47–53`]. Questo fallback è fragile — se due funzioni sono dichiarate nello stesso scope, la seconda potrebbe sovrascrivere il contesto di ritorno della prima.
...
**Contesto di attivazione**: Istanziato una volta per unità di compilazione in `main.cpp:123`. Stateful — mantiene `symbols_`, `constraints_`, `errors_`, `typed_stmts_` come membro.
...
**`DEF-017` — `resolve_names` per `MainStmt` duplica binding**: In [`TypeChecker.cpp:116–117`], `main` viene registrato con tipo `void_()`, ma il nome "main" è hardcoded. Se il linguaggio dovesse supportare funzioni chiamate "main" dall'utente, ci sarebbe collisione.
...
`TypeBase` è puramente astratta — nessuna logica nel `.cpp`. I metodi `is_primitive()`, `is_numeric()`, `is_integer()` sono inline in `.hpp`:
...
**Performance**: I predicate inline (`is_numeric`, ecc.) eseguono uno switch su `TypeKind` ad ogni chiamata. Per tipi usati frequentemente, potrebbe valere la pena cacheare il risultato in un bitfield — ma il guadagno sarebbe marginale dato che l'operazione è O(1).
...
`TypeKind` è un enum class — nessuna conversione implicita rischiosa. I singleton restituiscono `const TypePtr&` — riferimento a `shared_ptr<const TypeBase>`, coerente con il resto del sistema.
...
`set_function_return_context()` [`SymbolTable.cpp:37–53`]: cerca prima per nome, poi fallback su qualsiasi function binding nello scope corrente. Il fallback è fragile.
...
**Robustezza**: `pop_scope()` dovrebbe assertare se vuoto. `define()` non dovrebbe creare scope implicitamente. Vedi **REC-012**.
...
**Expected ROI**: 5 — Impatto trasformativo: abilita type checking completo delle chiamate funzione, incluindo arity e tipo di ritorno.
