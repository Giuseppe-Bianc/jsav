...
namespace jsv {
    ...
    [[nodiscard]] std::expected<std::size_t, std::string> visual_column(std::string_view line, std::size_t byte_offset,
        ...
        while(pos < byte_offset && pos < line.size()) {
            ...
            if(code_point_count > k_max_code_points) {
                const std::string error_msg = FORMAT("Line exceeds maximum length of {} code points (actual: {})", k_max_code_points,
                                                     code_point_count);
                LERROR("{}", error_msg);
                return std::unexpected(error_msg);
            }
    ...
    [[nodiscard]] std::expected<std::pair<std::size_t, std::size_t>, std::string> marker_extents(std::string_view line,
        ...
        if(!leading_result.has_value()) { return std::unexpected(std::move(leading_result.error())); }
        std::size_t leading_spaces = leading_result.value() - 1;

        // FR-013: Minimum 1 caret guarantee
        if(start_byte >= end_byte) { return std::make_pair(leading_spaces, 1u); }
