...
namespace jsv::crypto {
    ...
    Sha256::Sha256() noexcept : state_(H_INIT), count_(0), bufferIdx_(0) { buffer_.fill(0); }
    ...
    void Sha256::transform(const std::uint8_t *block) noexcept {
        ...
        for(std::size_t t = 0; t < 16; ++t) {
            w.at(t) = (C_UI32T(block[t * 4 + 0]) << 24) | (C_UI32T(block[t * 4 + 1]) << 16) | (C_UI32T(block[t * 4 + 2]) << 8) |
                      (C_UI32T(block[t * 4 + 3]));
        }
    ...
    Sha256::Digest Sha256::finalize() noexcept {
        ...
        for(int i = 7; i >= 0; --i) {
            const auto b = C_UI8T((bitCount >> (C_UI64T(i) * 8)) & 0xFF);
            update({&b, 1});
        }

        Digest digest{};
        for(std::size_t i = 0; i < 8; ++i) {
            digest.at(i * 4 + 0) = C_UI8T((state_.at(i) >> 24) & 0xFF);
            digest.at(i * 4 + 1) = C_UI8T((state_.at(i) >> 16) & 0xFF);
            digest.at(i * 4 + 2) = C_UI8T((state_.at(i) >> 8) & 0xFF);
            digest.at(i * 4 + 3) = C_UI8T(state_.at(i) & 0xFF);
        }
    ...
    std::string Sha256::toHexString(const Digest &digest) {
        ...
        for(const auto b : digest) { res += FORMAT("{:02x}", b); }
        return res;
    }
