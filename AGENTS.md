...
|-----------|---------|---------------|-------------|
| `.devcontainer/` | **Development container configuration** — Reproducible local environment setup | **Consumed by**: VS Code Dev Containers, Docker tooling<br>**References**: project root layout and toolchain requirements | Keep environment definitions aligned with CMake/toolchain prerequisites. Do not add project logic here. |
| `.github/` | **Repository automation and governance** — Workflows, templates, prompts, agent config | **Consumed by**: GitHub Actions, issue/PR UX, repository automation | Workflow changes must preserve CI gates (build, test, static analysis, sanitizers). Keep templates consistent with contribution rules. |
| `.specify/` | **Specification system metadata** — memory, templates, workflow configuration | **Consumed by**: Spec workflow tooling and planning artifacts | Treat as process infrastructure. Changes must not contradict constitution and project governance documents. |
| `include/` | **Header root** — Public API (`include/jsav/`) plus internal shared headers (`include/jsavCore/`) | **Consumed by**: `src/`, `test/`, optional `fuzz_test/`<br>**Installed**: only public headers under `include/jsav/` | Preserve API boundary between public and internal headers. No circular include direction between `include/jsav/` and `include/jsavCore/`. |
| `include/jsav/` | **Installed public API headers** — Compiler-facing interfaces and umbrella headers | **Consumed by**: `src/jsav_Lib/`, `src/jsav/`, `test/`, downstream consumers<br>**May include**: selected `include/jsavCore/` utilities | Stable API surface: avoid breaking changes without deprecation path. Public symbols require documentation and compatibility review. |
| `include/jsavCore/` | **Internal shared headers** — Core utilities (logging, file I/O, format, timer, casts) | **Consumed by**: `src/jsav_Core_lib/`, `src/jsav_Lib/`, `src/jsav/`, `test/` | Internal-only (not installed). Must not depend on `include/jsav/` to avoid reverse coupling. |
| `src/` | **Implementation root** — Executable and library target sources | **Defines**: `jsav`, `jsav_core_lib`, `jsav_lib` via subdirectory `CMakeLists.txt` files | Respect target dependency direction and keep implementation split by responsibility. |
| `src/jsav/` | **Executable layer** — CLI entry point and orchestration | **Links against**: `jsav::jsav_lib`, `jsav::jsav_core_lib` | Keep orchestration-only logic in `main.cpp`; domain logic belongs in libraries. |
| `src/jsav_Core_lib/` | **Core library layer** — Foundational utilities implementation | **Depends on**: `include/jsavCore/`, third-party core deps<br>**Consumed by**: `src/jsav_Lib/`, `src/jsav/`, `test/` | Must remain dependency-light and reusable. No dependency on `src/jsav_Lib/`. |
| `src/jsav_Lib/` | **Compiler library layer** — Main compiler subsystems (lexer/parser/typechecker/etc.) | **Depends on**: `include/jsav/`, `include/jsavCore/`, `jsav::jsav_core_lib`<br>**Consumed by**: `src/jsav/`, `test/`, optional fuzz targets | Contains compiler business logic; may depend on core library, never the inverse. Maintain module boundaries by subsystem folders. |
| `test/` | **Test root** — Unit, constexpr, and benchmark coverage | **Links against**: `jsav::jsav_lib`, `jsav::jsav_core_lib`, Catch2<br>**Discovers**: `constexpr_tests`, `relaxed_constexpr_tests`, `tests` | Tests must be deterministic, isolated, and aligned with target responsibilities. Runtime tests stay in `test/tests.cpp`; constexpr tests in `test/constexpr_tests.cpp`. |
| `fuzz_test/` | **Fuzzing targets** — Robustness testing for malformed/randomized input | **Links against**: `jsav::jsav_lib` (+ LibFuzzer when enabled) | Optional build path; must stay idempotent and fast. No production code coupling from fuzz harnesses back into libraries. |
| `cmake/` | **Build infrastructure modules** — warnings, sanitizers, hardening, packaging, utilities | **Referenced by**: root and subproject `CMakeLists.txt` | Treat as shared infrastructure; edits require cross-platform verification and must remain side-effect free/idempotent. |
| `configured_files/` | **CMake template inputs** — generated configuration/header templates | **Consumed by**: CMake configure step<br>**Produces**: generated files in build tree | Keep templates declarative (`@VAR@` substitution). Generated outputs are build artifacts and must not be committed. |
| `scripts/` | **Automation scripts** — utility and code-generation helpers | **Consumed by**: developers and CI workflows | Scripts should be explicit about platform assumptions and input/output contracts; avoid hidden side effects on source tree. |
| `specs/` | **Specification artifacts** — feature specifications, plans, and design notes | **Referenced by**: contributors, reviewers, planning tooling | Keep specs synchronized with implementation decisions and architectural constraints. |
| `vn_files/` | **Language fixtures/samples** — `.vn` programs for examples and semantic/type-checker scenarios | **Consumed by**: manual validation, test scenarios, tooling experiments | Treat as fixtures: keep deterministic, versioned, and representative of language features under test. |
