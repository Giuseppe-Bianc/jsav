...
macro(jsav_supports_sanitizers)
    ...
    elseif ((CMAKE_CXX_COMPILER_ID MATCHES ".*Clang.*" OR CMAKE_CXX_COMPILER_ID MATCHES ".*GNU.*") AND NOT WIN32)
        ...
        # Quoted variable expansions for safety
...
macro(jsav_global_options)
    # Configure SIMD support detection
    include(cmake/Simd.cmake)
    check_all_simd_features()
    print_simd_support()

    # Enable IPO/LTO if requested
    if (jsav_ENABLE_IPO)
        include(cmake/InterproceduralOptimization.cmake)
        jsav_enable_ipo()
    endif ()

    # Re-check sanitizer support for hardening configuration
    jsav_supports_sanitizers()

    # Apply hardening globally if enabled and global propagation is requested
    if (jsav_ENABLE_HARDENING AND jsav_ENABLE_GLOBAL_HARDENING)
        include(cmake/Hardening.cmake)

        # UBSan minimal runtime is incompatible with certain sanitizers
        # Disable it if any conflicting sanitizer is enabled
        ...
        message(STATUS "Hardening config: ENABLE=${jsav_ENABLE_HARDENING}, UBSAN_MINIMAL=${ENABLE_UBSAN_MINIMAL_RUNTIME}, UBSAN_ENABLED=${jsav_ENABLE_SANITIZER_UNDEFINED}")
...
macro(jsav_local_options)
    ...
    add_library(jsav::options ALIAS jsav_options)

    target_compile_options(jsav_options INTERFACE
    ...
    set_target_properties(jsav_options PROPERTIES UNITY_BUILD "$<BOOL:${jsav_ENABLE_UNITY_BUILD}>")

    # Enable precompiled headers if requested
    # PCH can improve compile times for large projects with common headers
    if (jsav_ENABLE_PCH)
    ...
    # Enable ccache for compilation caching if requested
    ...
    # Enable linker warnings-as-errors if supported
    ...
    # Apply hardening locally (not propagated to dependencies)
