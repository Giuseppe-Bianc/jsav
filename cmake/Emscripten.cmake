...
set(jsav_WEB_DIR "${CMAKE_SOURCE_DIR}/web")
set(jsav_COI_WORKER "${jsav_WEB_DIR}/coi-serviceworker.min.js")
set(jsav_SHELL_TEMPLATE "${jsav_WEB_DIR}/shell_template.html.in")
set(jsav_INDEX_TEMPLATE "${jsav_WEB_DIR}/index_template.html.in")
...
if(EMSCRIPTEN)
  ...
  set(jsav_WASM_INITIAL_MEMORY "33554432" CACHE STRING
      "Initial WASM memory in bytes (default: 32MB)")
  set(jsav_WASM_PTHREAD_POOL_SIZE "4" CACHE STRING
      "Pthread pool size for WASM builds (default: 4)")
  set(jsav_WASM_ASYNCIFY_STACK_SIZE "65536" CACHE STRING
...
function(jsav_configure_wasm_target target)
  if(EMSCRIPTEN)
    ...
    set_property(GLOBAL APPEND PROPERTY jsav_WASM_TARGETS "${target}")
    set_property(GLOBAL PROPERTY jsav_WASM_TARGET_${target}_TITLE "${WASM_TITLE}")
    set_property(GLOBAL PROPERTY jsav_WASM_TARGET_${target}_DESCRIPTION "${WASM_DESCRIPTION}")
    set_property(GLOBAL PROPERTY jsav_WASM_TARGET_${target}_OUTPUT_NAME "${OUTPUT_NAME}")

    target_compile_definitions(${target} PRIVATE jsav_WASM_BUILD=1)
    ...
    target_link_options(${target} PRIVATE
      ...
      "-sUSE_PTHREADS=1"
      "-sPROXY_TO_PTHREAD=1"
      "-sPTHREAD_POOL_SIZE=${jsav_WASM_PTHREAD_POOL_SIZE}"
      # Enable asyncify for emscripten_sleep and async operations
      "-sASYNCIFY=1"
      "-sASYNCIFY_STACK_SIZE=${jsav_WASM_ASYNCIFY_STACK_SIZE}"
      # Memory configuration
      "-sALLOW_MEMORY_GROWTH=1"
      "-sINITIAL_MEMORY=${jsav_WASM_INITIAL_MEMORY}"
    ...
    if(EXISTS "${jsav_SHELL_TEMPLATE}")
      ...
      message(FATAL_ERROR "Shell template not found: ${jsav_SHELL_TEMPLATE}")
    endif()

    # Copy service worker to target build directory for standalone target builds
    if(EXISTS "${jsav_COI_WORKER}")
...
function(jsav_create_web_dist)
  ...
  get_property(WASM_TARGETS GLOBAL PROPERTY jsav_WASM_TARGETS)
  ...
  foreach(target ${WASM_TARGETS})
    get_property(TITLE GLOBAL PROPERTY jsav_WASM_TARGET_${target}_TITLE)
    get_property(DESCRIPTION GLOBAL PROPERTY jsav_WASM_TARGET_${target}_DESCRIPTION)
...
")
  ...
  if(EXISTS "${jsav_INDEX_TEMPLATE}")
    configure_file("${jsav_INDEX_TEMPLATE}" "${INDEX_OUTPUT}" @ONLY)
  else()
    message(WARNING "Index template not found: ${jsav_INDEX_TEMPLATE}")
  endif()
  ...
  foreach(target ${WASM_TARGETS})
    ...
    get_property(OUTPUT_NAME GLOBAL PROPERTY jsav_WASM_TARGET_${target}_OUTPUT_NAME)
