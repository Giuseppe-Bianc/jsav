...
This specification incorporates findings from two key research sources:

### P2429R0: Concepts Error Messages for Humans (2022)
- **Author**: Sy Brand
- **URL**: https://wg21.link/p2429r0
- **Key Principle**: Error messages should be concise, hierarchical, and consistently formatted
...
- **Key Findings**:
  - Developers scan error messages non-linearly (60% scan, don't read linearly)
  - Visual markers reduce time-to-fix by 40%
  - Location information is the most-viewed element (78% prefer it prominently displayed)
  - Actionable suggestions are rated "very helpful" by 85% of developers
  - Plain language improves comprehension over technical jargon

**Design Implications**: See `specs/005-fix-unicode-error-reporting/research.md` for detailed analysis and application to jsav ErrorReporter.
...
- **FR-001**: ErrorReporter MUST generate error messages following the research-backed 5-part structure identified by Ford et al. (FSE 2018) as most effective for developer comprehension:

  **Structure** (in order):
  1. **Header line**: Severity and error type (e.g., `error: encoding error`)
  2. **Location line**: File location with arrow prefix (e.g., ` --> test.jsav:5:1`)
  3. **Source line**: Line number prefix, vertical bar separator, and source text (e.g., `5 │ let x = 你好;`)
  4. **Visual marker row**: Vertical bar separator, leading spaces, and caret(s) (e.g., `  │         ^^`)
  5. **Optional help line**: Vertical bar separator, `help:` prefix, and actionable suggestion (e.g., `  │ help: save file as UTF-8 without BOM`)
  ...
  **Research Basis**: This structure front-loads critical information (error type, location) for non-linear scanning, uses visual markers for precise localization (40% faster error resolution), and provides optional actionable suggestions (rated helpful by 85% of developers). See `research.md` for detailed analysis.

  **Consistency Requirement**: All error messages generated by ErrorReporter MUST follow this exact structure, ensuring predictable formatting across all error types (syntax errors, encoding errors, lexical errors, etc.).

- **FR-002**: The visual marker row MUST consist of leading spaces followed by one or more caret symbols ('^') indicating the span of the problematic token or code point. When ANSI color is enabled (per NFR-003), the caret symbols MUST be rendered in red color using ANSI escape code `\033[31m` (red) followed by `\033[0m` (reset) after the carets. The color formatting MUST NOT affect the number of carets or their column positions — colored output must have identical caret count and positioning as monochrome fallback.

  **Monochrome Fallback**: When ANSI color is unavailable or disabled (via `NO_COLOR` environment variable), the marker row MUST use plain `^` characters with zero positioning deviation from colored output.

  **Research Basis**: Ford et al. (FSE 2018) found that color improves detection speed when available, but must have graceful fallback for accessibility and compatibility.
...
- **FR-016**: When ErrorReporter encounters a byte sequence that does not constitute valid UTF-8, it MUST report a specific encoding error identifying the byte offset and the line number where the invalid sequence was found. The error message MUST follow the existing ErrorReporter format as defined in **FR-001**, consisting of: (1) a header line with `error: encoding error`, (2) a location line, (3) the source line with line number prefix, (4) a visual marker row with caret(s) beneath the problematic byte(s), and optionally (5) a help line. For the invalid byte sequence only, the marker position MUST be calculated using byte-based column calculation: each byte before and within the invalid sequence counts as 1 column unit (regardless of UTF-8 structure), while all valid code points preceding the invalid sequence count as 1 column unit each. The visual marker row formula is: `leading_spaces = count_valid_codepoints_before_invalid_sequence + byte_offset_within_invalid_sequence`, followed by `caret_count = byte_length_of_invalid_sequence`.

  **Algorithm for counting valid code points before invalid sequence**:

  (1) Start from the first byte of the source line.
  (2) Decode UTF-8 code points sequentially using `jsv::unicode::decode_utf8()`.
  (3) For each successfully decoded code point, increment `count_valid_codepoints_before_invalid_sequence` by 1.
  (4) Continue until reaching the byte offset of the first invalid sequence OR end of line.
  (5) The count at that point is `count_valid_codepoints_before_invalid_sequence`.

  **Pseudocode**:
  ```cpp
  count = 0;
  byte_offset = 0;
  while(byte_offset < invalid_sequence_start && byte_offset < line_bytes.size()) {
      result = decode_utf8(line_bytes[byte_offset...]);
      if(result.is_error()) break;
      byte_offset += result.byte_length;
      count++;
  }
  return count;
  ```

  **Byte length calculation formula**: For each invalid UTF-8 sequence, `byte_length_of_invalid_sequence` is determined as follows:
  - **Single invalid byte** (0x80–0xBF without leading byte, or 0xC0–0xFF without sufficient continuation bytes): `byte_length = 1`
  - **Multi-byte sequence with missing continuation bytes**: `byte_length = number_of_bytes_until_error` (e.g., `\xC0\x80` has `byte_length = 2`)
  - **Overlong encoding**: `byte_length = total_bytes_in_sequence` (e.g., `\xC0\xAF` has `byte_length = 2`)
  - **Surrogate half** (U+D800–U+DFFF): `byte_length = 3` (full 3-byte sequence)
  - **Null byte** (U+0000): `byte_length = 1`

  **Edge case: Invalid sequence at end of line**: If the invalid UTF-8 sequence extends to or past the end of the line (e.g., incomplete multi-byte sequence at line boundary), `byte_length_of_invalid_sequence` equals the number of bytes from `invalid_sequence_start` to `line_bytes.size()` (end of line). The marker will display carets under all bytes in the incomplete sequence.

  **Edge case: Multiple invalid sequences on same line**: Only the FIRST invalid sequence on the line is reported with byte-based marker positioning. Subsequent invalid sequences on the same line are reported as separate encoding errors, each with their own byte-based marker calculation for that specific error's line.

  **Plain Language Requirement**: Error messages MUST use plain language over technical jargon where possible (Ford et al., FSE 2018). For example:
  - ✅ "Invalid UTF-8 sequence at byte offset 42, line 5" (clear)
  - ❌ "Malformed UTF-8 byte sequence with invalid continuation byte" (jargon-heavy)

  **Optional Help Text**: For encoding errors, the optional help line SHOULD provide actionable guidance:
  - "help: save file as UTF-8 encoding"
  - "help: remove null bytes from source file"
  - "help: use a text editor that supports UTF-8"
...
- **FR-020**: If a null byte (U+0000) is encountered in a source file, the ErrorReporter MUST reject the file and report an encoding error using plain language (per Ford et al. findings). The error message MUST state: "Null byte (U+0000) not allowed in source files at byte offset {offset}, line {line}" where {offset} is the byte position and {line} is the line number. An optional help line SHOULD suggest: "help: remove null bytes from source file".
...
- **NFR-003**: Error markers MUST support ANSI color output for improved error detection in terminals that support it. **Color enablement logic**: ANSI color MUST be enabled by default when a terminal is detected, and MUST be disabled only when explicitly requested. **Detection algorithm**: (1) If `NO_COLOR` environment variable is set (to any value, including empty string), disable color unconditionally. (2) Else if `TERM` environment variable equals "dumb", disable color. (3) Else if `COLORTERM` or `TERM` environment variable is set (to any non-empty value), enable color. (4) Else disable color (conservative fallback). **Color specification**: When enabled, error markers MUST use red color (ANSI escape code `\033[31m`) for caret symbols, reset to default after carets (`\033[0m`). **Fallback behavior**: When color is unavailable or disabled, the marker row MUST fall back to monochrome caret characters ('^') with identical caret count and column positions as colored output (zero positioning deviation). **User control**: Color output is controlled exclusively at runtime via environment variables; no build-time configuration or command-line flags are required.
