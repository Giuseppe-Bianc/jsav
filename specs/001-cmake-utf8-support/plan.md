...
Add UTF-8 support to the jsav C++23 compiler project by introducing three CMake modules (`cmake/Utf8Compiler.cmake`, `cmake/Utf8Console.cmake`, `cmake/Utf8BomStrip.cmake`) and one configured header (`configured_files/include/jsav/utf8_console.hpp.in`). The implementation uses compiler built-ins (`/utf-8` for MSVC, `-finput-charset=UTF-8 -fexec-charset=UTF-8` for GCC/Clang), Win32 `SetConsoleOutputCP(CP_UTF8)` API, and CMake's native file/string commands for BOM handling. Integration occurs through `jsav_ENABLE_UTF8` option (default ON) wired into existing `ProjectOptions.cmake` pipeline.
...
**Performance Goals**: Standard compiler performance; UTF-8 flag overhead negligible (<1% build time impact)
**Constraints**: Zero runtime overhead for non-UTF-8 paths; fail-fast on UTF-8 console initialization failure; no additional third-party dependencies
...
|------------------------------|-----------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **I. Platform Independence** | ✅ PASS   | UTF-8 handling uses cross-platform compiler flags (`-finput-charset=UTF-8 -fexec-charset=UTF-8` for GCC/Clang, `/utf-8` for MSVC). Windows-specific `SetConsoleOutputCP` is encapsulated in optional runtime header, not forced on all targets. Linux/macOS require no runtime initialization. |
| **II. Visual Studio 2026 Compatibility** | ✅ PASS | MSVC 2022+ with `/utf-8` flag is fully supported. All CMake modules use standard CMake 3.29+ commands. No deprecated MSVC features used.                                                                                               |
| **III. C++ Core Guidelines Compliance** | ✅ PASS | No raw `new`/`delete`; uses CMake functions and configured files. Generated header uses `std::exit` and `fmt::print` (approved dependencies). Function complexity trivial (<10 lines). Zero static analysis violations expected.        |
...
|------------------------------|-------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **I. Platform Independence** | ✅ PASS (Confirmed) | Design artifacts confirm: CMake modules use cross-platform commands; runtime header uses `#ifdef _WIN32` for platform-specific code; Linux/macOS behavior is no-op.                                                                                                            |
| **II. Visual Studio 2026 Compatibility** | ✅ PASS (Confirmed) | Generated header template uses only standard C++23 and approved Win32 API; no deprecated features introduced.                                                                                                                                                                 |
| **III. C++ Core Guidelines Compliance** | ✅ PASS (Confirmed) | data-model.md confirms: `init_console()` is 15 lines (within ≤100 limit), cyclomatic complexity = 1 (within ≤15 limit), 0 parameters (within ≤6 limit). CMake functions are ≤40 lines each.                                                                                   |
...
├── Utf8Console.cmake       # Console output setup
└── Utf8BomStrip.cmake      # BOM detection/stripping

configured_files/
└── include/
    └── jsav/
        └── utf8_console.hpp.in  # Generated runtime header

src/
└── jsav/                   # Main executable (unchanged, will include header)

test/
└── tests.cpp               # Extended with [utf8] test cases
```

**Structure Decision**: Single project structure (Option 1). UTF-8 support implemented as CMake modules and configured header, integrated into existing build system. No new subdirectories required beyond existing project layout.
