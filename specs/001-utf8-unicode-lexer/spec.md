...
- **FR-002**: The lexer MUST recognize identifiers that start with any Unicode XID_Start character and continue with zero or more XID_Continue characters.
- **FR-003**: The lexer MUST accept any valid UTF-8 character within string literals delimited by double quotes.
- **FR-004**: The lexer MUST accept any valid UTF-8 character within character literals delimited by single quotes.
- **FR-005**: The lexer MUST decode `\uXXXX` escape sequences where XXXX is exactly 4 hexadecimal digits representing a code point in the Basic Multilingual Plane (BMP).
- **FR-006**: The lexer MUST decode `\UXXXXXXXX` escape sequences where XXXXXXXX is exactly 8 hexadecimal digits representing any valid Unicode code point (U+0000 to U+10FFFF).
- **FR-007**: The lexer MUST recognize line comments starting with `//` and continuing to the next line ending.
- **FR-008**: The lexer MUST recognize block comments delimited by `/*` and `*/` that may span multiple lines.
- **FR-009**: The lexer MUST report a lexical error when encountering malformed UTF-8 sequences (e.g., continuation byte without start byte, incomplete sequence at end of file).
- **FR-010**: The lexer MUST report a lexical error when encountering overlong UTF-8 encodings (e.g., encoding ASCII characters using multi-byte sequences).
- **FR-011**: The lexer MUST report a lexical error when encountering UTF-8 sequences that decode to surrogate halves (U+D800 to U+DFFF).
- **FR-012**: The lexer MUST report a lexical error when encountering `\uXXXX` or `\UXXXXXXXX` escape sequences with invalid hexadecimal digits.
- **FR-013**: The lexer MUST report a lexical error when encountering `\uXXXX` or `\UXXXXXXXX` escape sequences that decode to surrogate halves or exceed U+10FFFF.
- **FR-014**: The lexer MUST track source positions using byte-based offsets from the start of the source.
- **FR-015**: The lexer MUST preserve the original UTF-8 byte sequence in token text for accurate error reporting and source mapping.
- **FR-016**: The lexer MUST recognize LF (`\n`, 0x0A) as a line ending.
- **FR-017**: The lexer MUST recognize CRLF (`\r\n`, 0x0D 0x0A) as a single line ending.
- **FR-018**: The lexer MUST recognize CR (`\r`, 0x0D) as a line ending.
- **FR-019**: The lexer MUST correctly increment line numbers when encountering any recognized line ending style.
- **FR-020**: The lexer MUST handle identifiers containing combining diacritical marks as part of the identifier (e.g., `café` with combining acute accent is a single identifier).
...
- **Token**: A lexical unit produced by the lexer, containing token type, the original UTF-8 byte sequence, and source position information (byte offset, line number, column).
...
- **SC-001**: The lexer correctly tokenizes source files containing identifiers from at least 10 different Unicode scripts (Latin, Greek, Cyrillic, Arabic, Hebrew, Devanagari, Bengali, Thai, CJK, Hangul) with 100% accuracy.
- **SC-002**: The lexer correctly decodes all valid `\uXXXX` escape sequences (65,536 possible values excluding surrogate halves) with 100% accuracy.
- **SC-003**: The lexer correctly decodes all valid `\UXXXXXXXX` escape sequences (1,114,112 possible Unicode scalar values) with 100% accuracy.
- **SC-004**: The lexer reports a lexical error for 100% of invalid UTF-8 sequences in the Unicode 15.0 Conformance Test Suite.
- **SC-005**: The lexer reports a lexical error for 100% of overlong UTF-8 encodings in test cases (all possible overlong forms for ASCII, BMP, and supplementary characters).
- **SC-006**: The lexer reports a lexical error for 100% of surrogate half inputs (both direct UTF-8 encoding and via escape sequences).
- **SC-007**: Source position tracking reports byte offsets within ±0 bytes of actual position for all tokens in test files.
- **SC-008**: Line number counting is accurate (±0 lines) for source files with LF, CRLF, and CR line endings.
- **SC-009**: The lexer processes UTF-8 source files with performance within 10% of ASCII-only lexing performance for files of equivalent byte length.
- **SC-010**: 100% of test cases pass for mixed-script identifiers (identifiers combining characters from multiple Unicode scripts).
...
- The lexer does not perform Unicode normalization on identifiers (e.g., `café` with precomposed é vs. combining accent are treated as different identifiers).
